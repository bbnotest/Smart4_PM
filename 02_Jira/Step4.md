...
> 🎯 **개요** — **스프린트**를 시작하고 **보드**에서 진행 상황을 옮기며, 이슈를 **담당자 사이로 전달(이관)** 해 완료까지 흐르게 합니다.
...
보드 드래그 말고 **이슈 상세에서도** 상태를 바꿀 수 있고, 역할이 바뀌면 **담당을 넘깁니다(이관)**.

1. 이슈 열기 → 상단 **상태 버튼**(`할 일` ▾ → `진행 중`) 클릭 — 보드 이동과 같은 효과.
2. **담당 이관(핸드오프)**: 개발이 끝나면 이슈의 **`Assignee`를 QA로 변경** + 상태를 **`검토 중`** 으로 → QA에게 전달.
3. QA가 통과시키면 **`완료`** 로. 이게 이슈 하나의 **생명주기**입니다.

```mermaid
graph LR
    C["생성<br/>(3단계)"] --> AS["배정·전달<br/>@멘션"] --> IP["진행 중<br/>DEV"] --> HO["이관<br/>DEV→QA"] --> RV["검토 중<br/>QA"] --> DN["완료"]
```

> 🙋 **핵심**: 상태(어디까지)와 담당(누구 손)이 함께 움직입니다. "DEV가 진행 중 → QA로 넘기며 검토 중" 이 한 박자가 곧 **전달 프로세스**예요.
...
- [ ] 이슈를 **다른 담당자(QA)로 이관**하며 상태를 옮겨봤다
