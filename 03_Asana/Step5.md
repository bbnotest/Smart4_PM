...
1. 태스크 상세 패널 → **`Tags`**(태그) 칸에 입력: `우선-High` / `우선-Medium` / `우선-Low`
2. 태그는 **색**으로 구분되고, 태그를 누르면 **그 태그가 달린 작업 전체를 모아** 볼 수 있습니다.
3. 유형 태그도 유용합니다: `버그` / `아트` / `기획`.
...
> **Pixel Dungeon 맥락** — Asana의 진짜 무기는 **비개발 직군과의 매끄러운 소통**입니다. 아트·기획이 코멘트·@멘션으로 바로 대화하니 별도 메신저를 덜 거칩니다. 무료에선 **분류=태그**가 커스텀 필드를 대신합니다.
