# 🟦 Jira · 5단계 — Timeline으로 일정 잡기

> 🎯 **개요** — 에픽들의 기간·순서·마일스톤을 **Timeline(무료 간트)** 에 그려 전체 일정을 한눈에 만듭니다.

<div class="scenario">
<span class="who">🎬 상황 · Pixelforge Studio · 1주차 금요일</span>
대표가 부른다. "다음 주 <b>투자자 미팅</b>이 있어요. '8주 안에 이렇게 만든다'는 <b>로드맵</b> 한 장 만들어 줄래요?" 에픽 일정과 마일스톤(M1~M4)을 Timeline에 막대로 그린다.
</div>

📍 [← 4단계](Step4.md) · [6단계 →](Step6.md)

---

## Timeline = 무료로 쓰는 간트차트

Asana·Trello에선 유료인 간트를, Jira는 **무료로** 제공합니다. 그래서 간트는 여기서 배워두면 이득이에요.

1. 왼쪽 **`Timeline`** 열기
2. 각 **에픽 막대**의 좌우 끝을 **드래그**해 기간 설정 (아래 표대로)
3. 막대 끝을 다음 에픽 앞에 연결 → **의존성(화살표)**
4. 마일스톤/스프린트 표시를 켜서 M1~M4 확인

| 에픽 | 기간 | 마일스톤 |
|---|---|---|
| E1 기획 | 7/06–7/17 | |
| E2 코어 | 7/06–7/24 | **M1**(7/17) |
| E3 던전 | 7/20–7/31 | **M2**(7/31) |
| E5 UI·메타 | 7/27–8/14 | **M3**(8/14) |
| E7 QA·출시 | 8/10–8/28 | **M4**(8/28) |

완성하면 이런 모습입니다 👇

![Jira Timeline 목업](../assets/jira_timeline_mockup.svg)

> 🖼️ 공식 스크린샷 자리 — Timeline
> 출처: https://www.atlassian.com/agile/tutorials/how-to-do-scrum-with-jira

---

## ✅ 확인

- [ ] 에픽들이 시간축 막대로 배치돼 있다
- [ ] 마일스톤(M1~M4)이 표시된다

---

👉 다음: **[6단계 · 리포트 & 마무리](Step6.md)**
