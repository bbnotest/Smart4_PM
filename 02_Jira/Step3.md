...
## C. 담당자에게 전달 — 배정·코멘트·@멘션

이슈는 **만든 뒤 사람에게 전달**돼야 일이 굴러갑니다. Jira의 기본 전달 방식:

1. **담당자(Assignee) 지정** → 그 사람에게 **알림**이 갑니다("당신 일입니다").
2. 이슈 상세 하단 **코멘트**에 요청·맥락을 남기고 **`@이름`(멘션)** → 콕 집어 전달.
   - 예: `US-05 던전 생성`에 코멘트 `@아트 던전 타일셋 먼저 필요해요` → 아트팀에 알림.
3. 배정·@멘션 알림은 상대의 **알림함**에 쌓여 "내가 볼 일"이 분명해집니다.

> 🙋 **전달 = 배정 + 코멘트·@멘션**. 말로 "이거 해주세요" 하는 대신 이슈에 남기면 **기록**으로 남습니다.
