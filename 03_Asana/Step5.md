...
1. 태스크를 열고 → 우측 위 **`...`(더보기) → `태그 추가`** → 상세 패널에 **`태그` 칸**이 생깁니다
2. 칸에 입력해 만들기: `우선-High` / `우선-Med` / `우선-Low` → **`'…' 태그 생성`** → **색 선택**
...
- **협력자(Collaborators)·팔로워**: 태스크에 사람을 추가하면 **담당이 아니어도** 변경 알림을 받습니다. 코멘트엔 **👍 좋아요**로 "확인했어요"를 가볍게 표시.
