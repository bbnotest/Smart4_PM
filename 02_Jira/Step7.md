...
> 🎯 **개요** — **작업(Task)** 과 다른 **이슈(버그)** 를, QA팀이 어떻게 **등록·전달·처리·확인**하는지 익힙니다. 출시 전 QA 기간의 실제 흐름입니다.
...
> 🙋 Jira에선 Story·Task·Bug가 모두 "이슈 타입"이지만, 우리는 **계획된 작업**과 **QA가 발견한 버그**를 구분합니다. **이 페이지의 "이슈"는 버그**를 뜻합니다.
...
위 항목으로 등록하면 이슈 상세 화면은 이렇게 보입니다 👇

![Jira 7단계 — Bug 이슈 상세(실제 화면)](jira_step7.svg)

> ▲ **①** 이슈 타입을 **버그(Bug)** 로 · **②** 설명(Description)에 **★핵심 3종(현상·재현 스텝·기대 결과)** · **③** 보고자(Reporter). 환경·심각도(갤럭시 S26 울트라 · Minor)도 설명에 함께 적었습니다.

### 🔗 이슈 링크 — 버그를 관련 작업과 잇기

버그를 혼자 두지 말고 **관련 이슈와 연결**하면 맥락이 삽니다. 이슈 상세의 **`연결`(Link) → 유형 선택 → 이슈 번호** 입력:

| 링크 유형 | 뜻 | 예 |
|---|---|---|
| **blocks / is blocked by** | 막음 / 막힘 | 이 버그가 **`M2 출시`를 막음(blocks)** |
| **relates to** | 그냥 관련 | `US-07 게임오버 화면`과 관련 |
| **duplicates** | 중복 | 같은 버그면 하나로 합치고 닫기 |

> 🙋 에픽 연결(부모-자식)과 **이슈 링크는 다릅니다.** 에픽은 "큰 묶음 소속", 링크는 "이슈끼리 가로 연결(막음·관련·중복)"이에요. **blocks** 링크는 **[8단계 JQL](Step8.md)** 에서 "출시를 막는 블로커"를 추리는 핵심입니다.
...
> 🔧 **Affects version vs Fix version** — 등록 때 적은 `Affects version`(버그가 **나타난** 빌드, 예 `v1.0.3`)과 짝으로, 고친 뒤엔 **`Fix version`(이 버그가 **고쳐지는** 릴리스, 예 `v1.0.4`)** 를 채웁니다. 두 버전은 `프로젝트 → 릴리스(Releases)` 에서 관리되고, "이번 릴리스에 무슨 버그가 고쳐졌나"가 자동으로 모입니다.
