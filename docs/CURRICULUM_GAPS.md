...
> ✅ **2026-06-14 반영 완료** — 🔴 **우선 항목 11개** + 🟡 **선택 항목 14개** 전부 각 트랙에 반영 완료(맨 아래 "반영 로그" 참고).
...
🔴 **우선 항목 11개 전부 반영.** 각 툴 자기 내용으로만, 무료 기능만, 교차툴 의존 없음.

**Jira** (Step3 집중)
- 레이블(Labels) — `B. …+레이블` 불릿 + `## 🏷️ 레이블로 한 번 더 분류` + ✅ 항목
- 이슈 상세 한 바퀴(Description 포함) — `## 🔍 이슈 하나 열어보기 — 상세 패널 한눈에` 표
- Priority — `### 🚦 우선순위는 어떻게 넣나요?` (정식 필드 권장 + `p1·p2·p3` 레이블 대체). 8·10단계 `priority = Highest`와 정합 맞춤

**Asana**
- 태스크 완료(Mark complete) — Step3 `## C. 끝낸 일은 ✓ 완료 처리` + ✅
- Inbox·알림 — Step5 `## C. 받은 편지함(Inbox)` + ✅
- 반복 태스크 + 멀티홈잉 — Step7 `## D. 한 걸음 더 — 멀티홈잉 · 반복 태스크` + ✅

**Trello**
- 보드 공유·초대·공개범위 — Step1 `## C. 보드에 팀 초대 + 공개 범위` + ✅
- 시작일 + 마감 알림 — Step4 `### ③ 날짜 — 시작일·마감일·알림` + ✅
- 카드 Description + 마크다운 — Step5 `### 📝 설명(Description)` (맨 앞) + ✅
- 카드 보관(Archive) vs 삭제(Delete) — Step6 `## D. 끝난 카드는 보관(Archive)` + ✅

### 🟡 선택 항목 반영 (추가 패스)

각 툴 자기 내용으로만, 무료 기능만, 교차툴 의존 없음.

**Jira**
- 백로그↔보드 멘탈모델 · 필터바·스윔레인 — Step4 B
- 스프린트 종료(Complete)·이월 + Scrum/Kanban — Step4 `## D. 스프린트 종료 & 미완료 이월`
- 이슈 링크(blocks/relates/duplicates) — Step7 `### 🔗 이슈 링크`
- Affects vs Fix version·Releases — Step7 D

**Asana**
- 워크플로 갤러리 템플릿 — Step1 🙋
- 시작일(Start date) vs 마감일 — Step3 B 🙋
- 협력자/팔로워·코멘트 좋아요 — Step5 B
- 단축키(Tab+M…)·모바일 My Tasks — Step7 ⚡ 박스

**Trello**
- 카드 커버(Cover) — Step4 `### ④ 커버`
- 검색 연산자(@me/label:/due:)·전역검색 vs 보드필터·Watch 구독 — Step6 C
- 카드 Move·Copy(이월) — Step6 D
