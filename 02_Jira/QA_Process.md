...
📍 [← 6단계 · 리포트](Step6.md) · [7단계 · JQL →](Step7.md)
...
- **[7단계 · JQL·필터](Step7.md)** → `issuetype = Bug AND statusCategory != Done AND priority = Highest` 로 **출시 블로커 버그**만 저장해 매일 봄.
- **[8단계 · 자동화](Step8.md)** → `Bug 생성 → QA 라벨·담당 자동 지정`, `재발(Reopen) → 팀장 알림`.
- **[9단계 · 대시보드](Step9.md)** → **심각도별 버그 분포(파이)** + **미해결 버그 수(결과 필터)** 로 **QA 현황판**.

> 🔁 이렇게 **실무(이 페이지) → 응용(7~9단계)** 으로 이어집니다.

---

## 🎮 현장 감각 — 게임 PM은 이렇게

> **Pixel Dungeon 맥락** — 출시 전 QA 기간엔 버그 이슈가 쏟아집니다. **재현 스텝이 명확한 이슈**일수록 빨리 고쳐져요. PM은 우선순위/심각도로 "출시 블로커 먼저"를 정리하고, QA↔개발 핑퐁(재배정)이 매끄럽게 돌게 합니다.
...
- 다음: **[7단계 · JQL & 필터](Step7.md)** — 여기서부터 **🟣 응용 단계**(필터·자동화·대시보드로 버그를 키웁니다).
